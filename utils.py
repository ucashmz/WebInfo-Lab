...
class Knn:
    ...
    def predict(self, simV, newM, averV, userId, movieId):
        kNear = []
        index = 0
        tmpV = np.copy(simV)
        while index < 5:
    ...
    def recommend(self, newM, sqrtV, averV):
        ...
        with open(os.path.join(self.out_path, self.testing_out), 'w', encoding='utf-8') as file:
            ...
            for record in cs:
                ...
                out = self.predict(simV, newM, averV, userId, int(record[1]))
                file.write(str(out)+"\n")
    ...
    def validation(self, newM, sqrtV, averV):
        ...
        for record in cs:
            ...
            if count % 1000 == 0:
                print(count/1000)
            if count == 50000:  # just run 5000 ratings
                break
